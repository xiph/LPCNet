...
{
  int i,j;
  int nb_vectors, nb_entries, ndim, total_dim;
  ...
  for (i=0;i<nb_vectors;i++)
  {
    int nearest = find_nearest(codebook, nb_entries, &pred[i*ndim], ndim, NULL);
    for (j=0;j<ndim;j++)
    {
  ...
  vq_train(delta, nb_vectors, codebook2, nb_entries, ndim);
  
  err=0;
  for (i=0;i<nb_vectors;i++)
  {
    int n1;
    n1 = find_nearest(codebook2, nb_entries, &delta[i*ndim], ndim, NULL);
    for (j=0;j<ndim;j++)
    {
      delta[i*ndim+j] = delta[i*ndim+j] - codebook2[n1*ndim+j];
      err += delta[i*ndim+j]*delta[i*ndim+j];
    }
  }
  ...
  {
    for (j=0;j<ndim;j++)
      fprintf(fout, "%f, ", codebook[i*ndim+j]);
    fprintf(fout, "\n");
  }
  fprintf(fout, "};\n\n");
  ...
  {
    for (j=0;j<ndim;j++)
      fprintf(fout, "%f, ", codebook2[i*ndim+j]);
    fprintf(fout, "\n");
  }
  fprintf(fout, "};\n\n");
